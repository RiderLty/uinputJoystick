...
def getScreenCapturer(display_num = -1 , zone = [(0,0) , (1920,1080)]):
    ...
    return lambda : cv2.cvtColor(np.array(sct.grab(monitor)), cv2.COLOR_BGRA2BGR)
...
def update_image(original_image , hsv_image):
...
# sc_img = cv2.imread("") #读取文件
while True:
    sc_img = screenCapturer() #获取BGR格式图像
    # sc_img = url2ImgNp("http://192.168.3.155:4443/screenraw")
    cv2.imshow('Display', update_image(sc_img , cv2.cvtColor(sc_img , cv2.COLOR_BGR2HSV  )))
    if cv2.waitKey(1) == 27:
        break
